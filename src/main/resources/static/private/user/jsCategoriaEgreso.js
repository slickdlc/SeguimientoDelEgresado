...
function cambiarEstado(element, e, id) {
	$.ajax({
		url : "categoriaE/cambiarEstado?id=" + id,
		type : "POST",
		dataType : 'json',
...
$('#btnAgregar').click(function(e) {
	e.preventDefault();
	frm.trigger("reset");
	frm.attr('action', 'categoriaE/insertar');
	$('#btnGuardar').text("Agregar");

	row.find('.card-title').text("Categoria de Egreso");
	row.find('.card-category').text("Agregar Categoria de Egreso");
		...
		function(e) {
			$(frm).validate({
				submitHandler : function(form) {
					...
					var categoria = {
							nombreCategoriaEgreso:$('#nombreCategoria').val(),
							estado:1
					};
					$.ajax({
						url : $(frm).attr("action"),
...
function obtener(element,e,id){
	location.href="/admin/categoriaE/editar?id="+id;
}